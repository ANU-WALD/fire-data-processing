...
zst_nc_path_file = '/g/data/ub8/au/FMC/c6/mosaics/deciles/zonal_stat' + '/' + plg_source + '_' + nc_var_short + '_zonal_stat.nc'
zst_csv_path_file = '/g/data/ub8/au/FMC/c6/mosaics/deciles/zonal_stat' + '/' + plg_source + '_' + nc_var_short + '_zonal_stat.csv'
...
exit()
df_all = pd.DataFrame(data={'plg_id2': plg_list_all, 'time': dt_list_all, 'min2': min_list_all, 'avg2': avg_list_all, 'max2': max_list_all}).set_index('time')
x = df_all.to_xarray()

x['plg_id'] = x.plg_id2.astype(np.float32)
x = x.drop(['plg_id2'])

x['min'] = x.min2.astype(np.float32)
x = x.drop(['min2'])

x['max'] = x.max2.astype(np.float32)
x = x.drop(['max2'])

x['avg'] = x.avg2.astype(np.float32)
x = x.drop(['avg2'])

print(zst_nc_temp_path_file)
x.to_netcdf(zst_nc_temp_path_file0)


# module load cdo
cmd_ncap2 = "ncap2 -s 'time=float(time)' " + zst_nc_temp_path_file0 + " " + zst_nc_temp_path_file
print(cmd_ncap2)
