    ...
    for year in range(2021,2022):
        ...
        if os.path.isfile('/g/data/ub8/au/FMC/c6/mosaics/deciles/fmc_c6_{}_dc.nc'.format(year)):
            ...
            if deciles.time.data[-1] == lfmc.time.data[-1]:
                os.system('qsub /g/data/xc0/user/scortechini/github/fire-data-processing/gadi_tasks/au_recurrent_qsubs/for_not_running_dec_everyday.qsub')
            else:
                os.system('qsub -v "year={0}" /g/data/xc0/user/scortechini/github/fire-data-processing/gadi_tasks/au_recurrent_qsubs/au_deciles_stats.qsub'.format(year))
        else:
            os.system('qsub -v "year={0}" /g/data/xc0/user/scortechini/github/fire-data-processing/gadi_tasks/au_recurrent_qsubs/au_deciles_stats.qsub'.format(year))
